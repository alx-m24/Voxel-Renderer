...
VoxelRenderer::VoxelRenderer(glm::vec3 chunkNum, glm::uvec3 chunkDimensions, float chunkSize)
{
	Init(chunkNum, chunkDimensions, chunkSize);
}

VoxelRenderer::VoxelRenderer(glm::vec3 chunkNum, glm::uvec3 chunkDimensions, float chunkSize, const std::vector<Voxel>& voxels)
{
...
void VoxelRenderer::InitShaders()
{
	...
	voxelShader.use();
	voxelShader.setUint("chunkNum", totalChunkNum);
...
{
	voxelFrameBuffer = FrameBuffer(FrameBufferAttachments::COLOR | FrameBufferAttachments::DEPTH, 1080, 1024);
	...
	for (int x = 0; x < chunkNum.x; ++x) {
		for (int y = 0; y < chunkNum.y; ++y) {
			for (int z = 0; z < chunkNum.z; ++z) {
				chunks.emplace_back(x, y, z);
			}
		}
	}

	std::vector<uint32_t> compressedChunks(totalChunkNum);
	for (int i = 0; i < totalChunkNum; ++i) {
		compressedChunks[i] = encodeChunk(chunks[i].x, chunks[i].y, chunks[i].z);
	}
	...
	CreateSSBO(emptyChunkSSBO, VoxelMemorySize * voxelNum, 2);
	...
	SetSubChunksShader.setFloat("chunkSize", chunkSize);
	SetSubChunksShader.setiVec3("chunkSubDivision", chunkSubDivision);
	SetSubChunksShader.setInt("pointLightNum", 0);

	std::vector<uint32_t> filledSubchunks(subDivisionCount, 1);

	for (int i = 0; i < totalChunkNum; ++i) {
		size_t offsetBytes = static_cast<size_t>(i) * subDivisionCount * sizeof(uint32_t);
...
void VoxelRenderer::Init(glm::vec3 chunkNum, glm::uvec3 chunkDimensions, float chunkSize)
{
	...
	this->totalChunkNum = chunkNum.x * chunkNum.y * chunkNum.z;

	this->voxelSize = glm::vec3(this->chunkDimensions) / this->chunkSize;

	this->InitShaders();
	this->InitBuffers();
}

void VoxelRenderer::Init(glm::vec3 chunkNum, glm::uvec3 chunkDimensions, float chunkSize, const std::vector<Voxel>& voxels)
{
	this->chunkDimensions = chunkDimensions;
	this->chunkSize = chunkSize;
...
void VoxelRenderer::Draw(const Camera& camera, Renderer& renderer)
{
	...
	// PostProcessing
	{
		BindTexture(0, voxelFrameBuffer.getAttachment(FrameBufferAttachments::COLOR)->id);
		BindTexture(1, voxelFrameBuffer.getAttachment(FrameBufferAttachments::DEPTH)->id);

		FrameOutput.Draw(&PostProcessingShader);
	}
}

void Lexvi::Extensions::VoxelRenderer::VoxelRenderer::OnResize(float width, float height)
{
	voxelFrameBuffer.ResizeFrameBuffer(width, height);

	voxelShader.use();
	voxelShader.setVec2("uResolution", width, height);

	PostProcessingShader.use();
	PostProcessingShader.setVec2("uResolution", width, height);
}
...
{
	float r = static_cast<float>((color >> 24) & 0xFFu) / 255.0;
	float g = static_cast<float>((color >> 16) & 0xFFu) / 255.0;
	float b = static_cast<float>((color >> 8)  & 0xFFu) / 255.0;
	float a = static_cast<float>(color & 0xFFu) / 255.0;
...
void VoxelRenderer::SendSubData(uint32_t* data, uint32_t voxelNum, size_t offset)
{
	UpdateSSBO(voxelSSBO, data, voxelNum * VoxelMemorySize, offset);
}
